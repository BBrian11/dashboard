...
import BranchContext, { initialState } from './BranchContext';
import providerReducer from './branchReducer';
...
import BranchOfficeService from '_mysource/services/BranchOfficeService';
...
const BranchProvider = ({ children }) => {
    ...
    const create = (branch) => {
        dispatchLoading();
        svc.current.save(branch)
            .then(results => {
                dispatch({ type: ACTIONS.CREATE, payload: results[0] });
                showSnackbar({ message: 'Sucursal creada correctamente', options: { variant: 'success' } });
            })
            .catch(error => {
                dispatchError(error);
            });
    ...
    const update = (branch) => {
        ...
        svc.current.save(branch)
            .then(results => {
                ...
                showSnackbar({ message: 'Sucursal actualizada correctamente', options: { variant: 'success' } });
            })
            .catch(error => {
                dispatchError(error);
            });
